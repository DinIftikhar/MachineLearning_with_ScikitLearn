...
filename_to_load = THIS_FILE_PATH + FILENAME
...
# Create web server
app = dash.Dash("diabetes_classification", external_stylesheets=external_stylesheets)

# In the additional file `helper_dami_homework` is hidden all webpage' structure
app.layout = deployed_model.app_html_layout
...
def update_histogram(colname, n_clicks):
    return deployed_model.update_histogram(colname, sample)
...
def update_scatter(col1, col2, n_clicks):
    return deployed_model.update_scatter(col1, col2, sample)
